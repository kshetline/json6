...
function ExtendedValue(functionName, parent) {
  ...
  functionValueName() {
    ...
    if ($) {
      ...
      if (util.isIdStartChar(buffer.charAt(0))) {
        return newToken('functionValueName', $[1]);
      }
    }

    throw invalidExtendedValueType(buffer);
  },
...
function push() {
  ...
  const extendedValueParent = parent instanceof ExtendedValue;
  let extendedValue = false;
  ...
  case 'functionValueName':
    value = new ExtendedValue(token.value, parent);
    extendedValue = true;
  ...
  if (root === undefined) {
    ...
    if (arrayParent) {
    ...
    else if (extendedValueParent) {
      value = optionsMgr.reviveFunctionValue(parent.name, value);
      ...
      if (value) {
        ...
        parseState = 'afterFunctionArgument';

        return;
      }

      throw unknownExtendedValueType(parent.functionName);
    }
    else {
      if (extendedValue) {
        value.key = key;
...
function invalidExtendedValueType(name) {
  column -= name.length;
  return syntaxError(`JSON-Z: invalid extended value type "${name}" at ${line}:${column}`);
}

function unknownExtendedValueType(name) {
  return syntaxError(`JSON-Z: unknown extended value type "${name}" at ${line}:${column}`);
}
