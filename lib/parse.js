...
const lexStates = {
  ...
  decimalInteger() {
    ...
    if (c === 'n') {
      read();
      return newToken(big.getBigIntType(), big.toBigInt((sign < 0 ? '-' : '') + buffer));
    }
    else if (c === 'd') {
      read();
      return newToken(big.getBigDecimalType(), big.toBigDecimal((sign < 0 ? '-' : '') + buffer));
    }
  ...
  hexadecimalInteger() {
    ...
    if (c === 'n') {
      read();
      return newToken(big.getBigIntType(), big.toBigInt(sign) * big.toBigInt(buffer));
    }
...
function push() {
  ...
  if (value !== null && (typeof value === 'object' && !big.isBigNumberInstance(value))) {
    stack.push(value);
...
//     return new Error(`JSON-Z: invalid parse state '${parseState}'`)
// }

// This code is unreachable.
// function invalidLexState (state) {
//     return new Error(`JSON-Z: invalid lex state '${state}'`)
// }
