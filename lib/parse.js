...
module.exports = function parse(text, reviver, newOptions) {
  ...
  key = undefined;
  root = undefined;
  reviveTypedContainers = !!options.reviveTypedContainers;
  ...
  if (reviveTypedContainers && util.isTypeContainer(root)) {
    root = optionsMgr.reviveTypeValue(root);
  }
  ...
  function pop() {
    ...
    if (current instanceof ExtendedType) {
      let revived = optionsMgr.reviveTypeValue(current.name, current.arg);
