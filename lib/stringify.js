...
module.exports = function stringify(value, replacer, space) {
  ...
  const expandedNumbers = options.expandedNumbers;
  const expandedPrimitives = options.expandedPrimitives;
  const expandedValues = options.expandedValues;
  ...
  function serializeProperty(key, holder, forArray) {
    ...
    if (expandedValues) {
      const functionValue = optionsMgr.stringifyAsFunctionValue(value, classPrefix, quoteString, expandedNumbers);
    ...
    if (big.isNumberOrBigNumber(value)) {
      return big.stringify(value, expandedNumbers, expandedPrimitives, preferredQuote);
    }
