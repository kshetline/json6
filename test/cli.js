...
tap.test('CLI', t => {
  t.test('converts JSON6 to JSON from stdin to stdout', t => {
    ...
    fs.createReadStream(path.resolve(__dirname, 'test.json6')).pipe(proc.stdin);
  });
  ...
  t.test('reads from the specified file', t => {
    const proc = child.spawn(
      ...
      [
        cliPath,
        path.resolve(__dirname, 'test.json6'),
      ]
    );
  ...
  t.test('validates valid JSON6 files', t => {
    const proc = child.spawn(
      process.execPath,
      [
        cliPath,
  ...
  t.test('validates invalid JSON6 files', t => {
    const proc = child.spawn(
      process.execPath,
      [
        cliPath,
        path.resolve(__dirname, 'invalid.json6'),
        '-v',
      ]
    );
    ...
    proc.stderr.on('end', () => {
      assert.strictEqual(error, "JSON6: invalid character 'a' at 1:1\n");
    });
